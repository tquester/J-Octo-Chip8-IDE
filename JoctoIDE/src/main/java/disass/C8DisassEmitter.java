...
public class C8DisassEmitter extends C8Emitter {
	...
	public int emitOpcode(byte[] code, int pos) {
			return emitOpcode(true, code, pos);
	}
	
	@Override
	public int emitOpcode(boolean withcomments, byte[] code, int pos) {

		if (withcomments) emitAliase(pos);
		autocomment = null;
		int addpc = 0;
		...
		try {
			...
			case 0x00:
				...
				case 0x01:
					int adr = low << 16;
					adr += read16bit(code[pos + 2] & 0xff, code[pos + 3] & 0xff);
					cmd = String.format("ldhi %s", lbladr(adr));
					addpc = 2;
					break;
				case 0x02:
					cmd = decode("ldpal", "02nn", high, low);
					break;
				case 0x03:
					cmd = decode("sprw", "03nn", high, low);
					break;
				case 0x04:
					cmd = decode("sprh", "04nn", high, low);
					break;
				case 0x05:
					cmd = decode("alpha", "050n", high, low);
					break;
				case 0x06:
					cmd = decode("digisnd", "060n", high, low);
					break;
				case 0x07:
					cmd = "stopsnd";
					break;
				case 0x08:
					cmd = decode("bmode", "080n", high, low);
					break;
				case 0x09:
					cmd = decode("ccol", "09nn", high, low);
					break;
				default:
					switch (low) {
					case 0x10:
			...
			case 0x0F:
				switch (low) {
				case 0x00:
					int adr = read16bit(code[pos + 2] & 0xff, code[pos + 3] & 0xff);
					cmd = String.format("i := long %s", lbladr(adr));
					addpc = 2;
					break;
				case 0x07:
			...
			if (disassFormat) {
				if (strlbl.length() < 16) {
					line = String.format("%04x %-16s %02x %02x", startpos, strlbl, high, low);
					if (addpc == 2)
						line += String.format(" %02x %02x ", code[pos + 2] & 0xff, code[pos + 3] & 0xff);
					else
						line += "       ";
					line += cmd;
				} else {
					...
					emitLine(pos, line);
					line = String.format("%04x %-16s %02x %02x", startpos, "", high, low);
					if (addpc == 2)
						line += String.format(" %02x %02x ", code[pos + 2] & 0xff, code[pos + 3] & 0xff);
					else
						line += "       ";
	...
	private String decode(String cmd, String pattern, int high, int low) {

		int opcode = high * 256 + low;
		int maskn = 0, maskx = 0, masky = 0;
		for (int i = 0; i <= 3; i++) {
			maskn <<= 4;
		...
		if (maskn != 0)
			cmd += String.format(" %d", opcode & maskn);

		return cmd;
	}

	private int read16bit(int b, int c) {
		return b * 256 + c;
	}
	...
	String bin(int data) {
		// String comment=Integer.toBinaryString(data).replaceAll("0", "
		// ").replaceAll("1", String.format("%c", 0x2588));
